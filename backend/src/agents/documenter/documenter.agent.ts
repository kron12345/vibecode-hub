...
const DEFAULT_SYSTEM_PROMPT = loadPrompt('documenter');
...
For code-level docs (README, API, JSDoc), keep \`wikiPage: false\` or omit it.`;
      ...
      const { wikiPages, screenshotWikiImages } = await syncDocsToWiki(
        wikiSyncCtx,
        docResult.files,
        diffs,
        writtenFiles,
  ...
  private async handleComplete(
    ...
    await cleanupScreenshots(
      ...
      this.resolveWorkspace.bind(this),
      this.logger,
    );
