...
export const AGENT_PRESETS: Record<string, AgentPreset> = {
  ...
  local: {
    name: 'Local (Ollama)',
    description:
      '3 core models optimized for 2×3090 / 48GB VRAM. Max quality, no API costs.',
    icon: 'hard-drive',
    roles: {
      INTERVIEWER: {
        provider: 'OLLAMA',
  ...
  cli: {
    ...
    description:
      'Codex Pro (unlimited) + Claude Max + Gemini Pro. Subscription-based, no API keys.',
    icon: 'terminal',
    roles: {
      // ── Planning & Design ──
